...
def build_feed_dict(input_tensors, input_batches):
    ...
    Args:
        ...
        input_batches: Numpy arrays that will be fed into the input tensors. (image, label)
    ...
    if should_tile:
        max_tensor_bsz = max([x for x in tensor_bszs if x is not None])  # 64
...
def get_tensors_from_checkpoint(sess, checkpoint_dir):
    ...
    meta_file = meta_files[-1]  # latest one
    ...
    new_saver.restore(sess, explicit_checkpoint_path)  # load model
...
def fetch_function(
    ...
    # feed_dict: replace tensor value in the graph, but why, length: 1
    fetched_data = sess.run(
        coverage_tensors + metadata_tensors, feed_dict=feed_dict
    )
    # fetched_data = sess.run(coverage_tensors + metadata_tensors)  # length: 100
